...
class ARKEngine:
    ...
    def discover_live_hosts(self, target_scope: str, timing_template: Optional[int] = None) -> List[Host]:
        logger.info(f"ARKEngine: Discovering live hosts in {target_scope} (Timing: T{timing_template if timing_template is not None else self.nmap_handler._get_validated_timing_template_value(None)})...")
    ...
    def scan_host_deep(
        ...
        scan_data = self.nmap_handler.run_port_scan_with_services(
            ...
            timing_template=timing_template # <<< PASS
        )
        # ... (rest of logic to populate host_obj remains the same) ...
    ...
    def scan_host_udp(self, host_obj: Host, top_ports: int = 100, include_version: bool = True, timing_template: Optional[int] = None): # Added timing
        default_timing_for_log = self.nmap_handler._get_validated_timing_template_value(None) # For logging default
        ...
        udp_ports: List[Port] = self.nmap_handler.run_udp_scan(
            ...
            timing_template=timing_template # Pass timing
        )
    ...
    def perform_basic_recon(
        ...
        timing_template: Optional[int] = None # <<< NEW PARAMETER
        ) -> List[Host]:
        default_timing_for_log = self.nmap_handler._get_validated_timing_template_value(None) # For logging default
        ...
        live_hosts: List[Host] = self.discover_live_hosts(
            target_scope,
            timing_template=timing_template # Pass timing to discovery
        )
        if not live_hosts:
    ...
    def _dataclass_to_dict_converter(self, obj: Any) -> Any:
        ...
        elif is_dataclass(obj) and not isinstance(obj, type): return asdict(obj)
    ...
    def export_to_csv(self, scan_results: List[Host], filename: str):
        ...
        headers = [
            "Host IP", "Hostname", "Host Status", "Port Number", "Port Protocol", "Port Status",
