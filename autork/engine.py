...
class ARKEngine:
    ...
    def scan_host_deep(
        ...
        """
        Performs TCP scan: ports, services, optionally OS and specified NSE scripts.
        Updates the Host object with results.
        """
        logger.info(f"ARKEngine: Performing TCP deep scan on {host_obj.ip} "
                    f"(top {top_ports} ports, OS={include_os_detection}, Scripts='{nse_scripts or 'None'}')...")

        scan_data = self.nmap_handler.run_port_scan_with_services(
            host_obj.ip,
            top_ports=top_ports,
            include_os_detection=include_os_detection,
            nse_scripts=nse_scripts # Pass the new parameter
        )

        # Update host object
        host_obj.ports = [p for p in host_obj.ports if p.protocol != 'tcp'] # Remove old TCP
        ...
        host_obj.host_scripts = scan_data.get("host_scripts", {}) # Update host scripts
    ...
    def perform_basic_recon(
        ...
        logger.info(f"ARKEngine: Starting full reconnaissance for target: {target_scope} "
                    f"(TCP ports={top_ports}, OS={include_os_detection}, Scripts='{nse_scripts or 'None'}', " # Updated log
